...
int main()
{
	...
	const PIO pio = pio0;
	const uint sm = 0;
	uint offset = pio_add_program(pio, &i2s32_program);
	double div = CPU_FREQ / (DAC_SAMPLING_RATE * DAC_BIT_DEPTH * 2 * 2);
	i2s32_program_init(pio, sm, offset, div, DATA_PIN, CLOCK_PIN_BASE);
	...
	uint16_t t=0;
	while (1) {
		value = sine_wave_table[t];
		t+=440*(1 << TABLE_BIT_DEPTH)/DAC_SAMPLING_RATE;
		pio_sm_put_blocking(pio, sm, (int32_t)value<<16);
	}
}